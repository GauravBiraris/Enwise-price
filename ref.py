...
if st.session_state.authenticated:
    ...
    if menu == "Pricing Policy":
        ...
        with tab2:
            ...
            strategy = st.radio("Competitor pricing strategy", ["Average", "Median", "Mode"])

            lock_prices = st.checkbox("Lock prices?")
            if lock_prices:
                products = pd.read_csv(products_file)
                products = st.dataframe(products[["Product ID", "Locked Price"]])

                if st.button("Save"):
                    products.to_csv(products_file, index=False)

            holiday_effect = st.checkbox("Consider holidays?")
            occasion_effect = st.checkbox("Consider occasions?")
            inventory_cycle = st.number_input("Inventory cycle (months)", min_value=1, max_value=12)
        ...
        if inventory:
            ...
            if st.button("Get Prices"):
                ...
                # Load pricing policy
                policy = pd.read_csv("pricing_policy.csv")
                min_margin = policy['Min Margin'].iloc[0]
                max_margin = policy['Max Margin'].iloc[0]
                # Load locked prices
                locked_prices = pd.read_csv("products.csv")
                ...
                output['Recommended Price'] = output['Locked Price']
                ...
                for i, row in output.iterrows():
                    ...
                    expiry = datetime.strptime(inventory.loc[i, 'Earliest Expiry'], '%Y-%m-%d').date()
                    remaining = (expiry - today).days

                    if remaining <= 60 and inventory.loc[i, 'Qty of Earliest Expiry'] > 0.4 * inventory.loc[i, 'Quantity in Stock']:
                        new_price = (95/100 * cost_price) + cost_price
                        output.loc[i, 'Recommended Price'] = new_price
                    if remaining <= 30 and inventory.loc[i, 'Qty of Earliest Expiry'] > 0.4 * inventory.loc[i, 'Quantity in Stock']:
                        new_price = (80/100 * cost_price) + cost_price
                        output.loc[i, 'Recommended Price'] = new_price
                    if remaining <= 15 and inventory.loc[i, 'Qty of Earliest Expiry'] > 0.4 * inventory.loc[i, 'Quantity in Stock']:
                        new_price = (50/100 * cost_price) + cost_price
                        output.loc[i, 'Recommended Price'] = new_price

                # Write output
                prices[['Product ID', 'Recommended Price', 'Validity']].to_csv("currentprices.csv", index=False)
